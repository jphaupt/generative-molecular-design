...
def test_rot_trans_invariance(single_batch, random_orthogonal_matrix, device):
    ...
    with torch.no_grad():
        ...
        _, _, mu1, log_var1, property_pred1, _ = outputs1
    ...
    with torch.no_grad():
        ...
        _, _, mu2, log_var2, property_pred2, _ = outputs2

    # Check that latent representations and property predictions are invariant
    assert torch.allclose(mu1, mu2, atol=1e-4)
    assert torch.allclose(log_var1, log_var2, atol=1e-4)
    assert torch.allclose(property_pred1, property_pred2, atol=1e-4)
